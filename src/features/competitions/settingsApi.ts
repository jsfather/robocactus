...
function personPayload(input: CompetitionPersonInput) {
  return {
    ...
    short_bio_en: input.short_bio_en || null,
    experience_years: input.experience_years == null ? null : Math.max(0, Math.min(100, Number(input.experience_years) || 0)),
    is_founder: input.is_founder === true,
    founder_badge_url: input.founder_badge_url || null,

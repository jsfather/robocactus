...
export function TeamRegistrationWizard({
  ...
  return (
    <div className="overflow-hidden rounded-[2rem] border border-sky-100 bg-white shadow-[0_24px_70px_rgb(18_76_98/0.12)]">
            ...
            return (
              ...
              >
                ...
                {openMemberIndex === index ? <div className="grid gap-3 p-4 md:grid-cols-2">
                  ...
                  <Select label="کشور" required value={member.country_code} onChange={(e) => patchMember(index, { country_code: e.target.value, is_foreign: e.target.value !== 'IR', nationality: e.target.value === 'IR' ? '' : 'اتباع' })}><option value="IR">ایران</option><option value="AF">افغانستان</option><option value="IQ">عراق</option><option value="OTHER">سایر</option></Select>
                  {member.country_code === 'IR' ? <Select label="تابعیت" required value={member.nationality} onChange={(e) => patchMember(index, { nationality: e.target.value })}><option value="">انتخاب تابعیت</option><option value="ایرانی">ایرانی</option><option value="اتباع">اتباع</option></Select> : null}
      ...
      {step === 3 && (memberPhotoType || memberIdentityType || teamOnlyDocs.length > 0) ? <section className="space-y-4"><div><h3 className="font-black text-slate-900">مدارک افراد و تیم</h3><p className="mt-1 text-xs leading-6 text-slate-500">پیش از ارسال، مدارک فعال را کنترل کنید. برای مشاهده کامل روی تصویر بزنید؛ برای اصلاح از دکمه مرحله قبل استفاده کنید.</p></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{(memberPhotoType || memberIdentityType) ? draft.members.map((member,index)=><article key={index} className="overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="flex items-center gap-3 border-b border-slate-100 p-3">{memberPhotoType ? <span className="grid size-9 place-items-center overflow-hidden rounded-lg bg-sky-50">{member.photo_url?<img src={member.photo_url} alt="" className="size-full object-cover"/>:(member.first_name||'?').slice(0,1)}</span> : null}<div><strong className="text-sm text-slate-900">{member.first_name} {member.last_name}</strong><p className="text-xs text-slate-500">{roleLabel(member.role)}</p></div></div>{memberIdentityType ? <div className="p-3"><PrivateImage path={member.national_id_doc_path} alt={`مدرک هویتی ${member.first_name}`} onOpen={setViewerUrl}/></div> : null}</article>) : null}{teamOnlyDocs.map(doc=><article key={doc.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white"><PrivateImage path={doc.file_path} alt={doc.doc_type} onOpen={setViewerUrl}/><p className="p-3 text-sm font-bold text-slate-700">{teamDocTypes.find(type=>type.code===doc.doc_type)?.label_fa??doc.doc_type}</p></article>)}</div></section> : null}

...
export type AttendanceClearance = {
  team_id: string; league_id: string; stage: 'members'|'technical'|'rules'|'confirmed'
  technical_status: 'locked'|'draft'|'pending'|'approved'|'rejected'
  technical_rejection_reason?: string|null; participant_note?: string|null; confirmed_at?: string|null
}
...
export async function fetchAttendance(teamId:string, leagueId:string) {
  const [flow, settings, members, files] = await Promise.all([
    backend.rpc('get_or_create_team_attendance',{p_team_id:teamId}),

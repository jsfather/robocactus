...
const baseRequired = ['first_name_fa', 'last_name_fa', 'first_name_en', 'last_name_en', 'birth_date', 'gender', 'email', 'province', 'city', 'country_code', 'nationality', 'postal_code', 'address']
...
export function AccountProfilePage() {
  ...
  useEffect(() => {
    setForm(profile ? { ...profile, country_code: profile.country_code || 'IR', is_foreign: profile.is_foreign ?? false } : profile)
  }, [profile])
  useEffect(() => {
  ...
  const missingProfileFields = useMemo(() => {
    ...
    const labels: Record<string, string> = {
      first_name_fa: 'نام فارسی', last_name_fa: 'نام خانوادگی فارسی', first_name_en: 'نام انگلیسی', last_name_en: 'نام خانوادگی انگلیسی',
      birth_date: 'تاریخ تولد', gender: 'جنسیت', email: 'ایمیل', phone: 'شماره موبایل', phone_verified_at: 'تأیید شماره موبایل',
      national_id: 'کد ملی', passport_number: 'شماره گذرنامه', province: 'استان', city: 'شهر', country_code: 'کشور', nationality: 'تابعیت',
      postal_code: 'کد پستی', address: 'نشانی کامل', company_name: 'نام مجموعه', company_national_id: 'شناسه ملی مجموعه', legal_representative_national_id: 'کد ملی نماینده قانونی',
    }
    const keys = new Set(Object.keys(participantErrors(form, rules)))
    if (documentsEnabled === true && docs.some((doc) => doc.is_required && !uploaded[doc.id])) keys.add('documents')
    return [...keys].map((key) => key === 'documents' ? 'مدارک الزامی' : labels[key] ?? key).filter((label, index, list) => list.indexOf(label) === index)
  }, [docs, documentsEnabled, form, rules, uploaded])
  const availableCities = useMemo(() => iranCities.filter((city) => city.province === form?.province), [form?.province, iranCities])
  const countryCode = form?.country_code || 'IR'
  ...
  return <PanelPage index="ID.01" title="پرونده هویتی شرکت‌کننده" description="اطلاعات صاحب حساب را کامل کنید.">
    {missingProfileFields.length ? <section className="mb-5 overflow-hidden rounded-[1.75rem] border border-amber-200 bg-gradient-to-br from-amber-50 via-white to-sky-50 p-5 shadow-sm sm:p-6" role="status" aria-live="polite"><div className="flex items-start gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-amber-100 text-xl font-black text-amber-700" aria-hidden="true">!</span><div className="min-w-0"><p className="text-xs font-black tracking-wide text-amber-700">تکمیل پرونده برای ادامه دسترسی</p><h2 className="mt-1 text-lg font-black text-slate-900">چند مورد از اطلاعات شما باقی مانده است</h2><p className="mt-2 text-sm leading-7 text-slate-600">موارد زیر را تکمیل و ذخیره کنید تا بتوانید به همه بخش‌های ثبت‌نام و پنل دسترسی داشته باشید.</p><div className="mt-4 flex flex-wrap gap-2">{missingProfileFields.map((item) => <span key={item} className="rounded-full border border-amber-200 bg-white px-3 py-1.5 text-xs font-bold text-amber-900">{item}</span>)}</div></div></div></section> : null}
    ...
    <form ref={formRef} className="space-y-5" noValidate onSubmit={(event) => void save(event)}>
      ...
      <PanelCard title="اطلاعات هویتی" description="فیلدهای ستاره‌دار الزامی هستند."><div className="grid gap-4 md:grid-cols-2">
        <Input label="نام فارسی" {...field('first_name_fa')} value={form.first_name_fa ?? ''} onChange={(e) => patch({ first_name_fa: withoutDigits(e.target.value) })} /><Input label="نام خانوادگی فارسی" {...field('last_name_fa')} value={form.last_name_fa ?? ''} onChange={(e) => patch({ last_name_fa: withoutDigits(e.target.value) })} />
        <Input label="نام انگلیسی" {...field('first_name_en')} dir="ltr" value={form.first_name_en ?? ''} onChange={(e) => patch({ first_name_en: withoutDigits(e.target.value) })} /><Input label="نام خانوادگی انگلیسی" {...field('last_name_en')} dir="ltr" value={form.last_name_en ?? ''} onChange={(e) => patch({ last_name_en: withoutDigits(e.target.value) })} />
        ...
        <div><Input label="شماره موبایل" {...field('phone')} dir="ltr" inputMode="numeric" maxLength={form.is_foreign ? 17 : 11} placeholder="09xxxxxxxxx" value={form.phone ?? ''} onChange={(e) => { patch({ phone: numericInput(e.target.value, form.is_foreign ? 17 : 11), phone_verified_at: null }); setOtpSent(false); setOtpChallengeId('') }} />{form.is_foreign ? <p className="mt-2 text-xs text-slate-500">برای شرکت‌کننده خارج از ایران، احراز هویت حساب از مسیر ایمیل انجام می‌شود.</p> : !form.phone_verified_at ? <div className="mt-2 flex gap-2">{otpSent ? <Input label="کد تأیید" dir="ltr" inputMode="numeric" value={otpCode} onChange={(e) => setOtpCode(numericInput(e.target.value, 6))} maxLength={6} /> : null}<Button type="button" variant="secondary" onClick={() => void handleProfileOtp()}>{otpSent ? 'تأیید کد' : 'ارسال کد'}</Button></div> : <p className="mt-2 text-xs font-bold text-emerald-600">شماره موبایل تأیید شده است.</p>}</div>
        <Select label="کشور" {...field('country_code')} value={countryCode} onChange={(e) => patch({ country_code: e.target.value, is_foreign: e.target.value !== 'IR', nationality: e.target.value === 'IR' ? '' : 'اتباع' })}><option value="IR">ایران</option><option value="AF">افغانستان</option><option value="IQ">عراق</option><option value="OTHER">سایر</option></Select>{countryCode === 'IR' ? <Select label="تابعیت" {...field('nationality')} value={form.nationality ?? ''} onChange={(e) => patch({ nationality: e.target.value })}><option value="">انتخاب تابعیت</option><option value="ایرانی">ایرانی</option><option value="اتباع">اتباع</option></Select> : null}
        {form.is_foreign ? <Input label="شماره گذرنامه" required error={errors.passport_number} dir="ltr" value={form.passport_number ?? ''} onChange={(e) => patch({ passport_number: e.target.value })} /> : <Input label="کد ملی" required error={errors.national_id} dir="ltr" inputMode="numeric" maxLength={10} value={form.national_id ?? ''} onChange={(e) => patch({ national_id: e.target.value.replace(/\D/g, '').slice(0, 10) })} />}
        {countryCode === 'IR' ? <Select label="استان" {...field('province')} value={form.province ?? ''} onChange={(e) => patch({ province: e.target.value, city: '' })}><option value="">انتخاب استان</option>{IRAN_PROVINCES.map((province)=><option key={province} value={province}>{province}</option>)}</Select> : <Input label="استان / منطقه" {...field('province')} value={form.province ?? ''} onChange={(e) => patch({ province: withoutDigits(e.target.value), city: '' })} />}{countryCode === 'IR' ? <Select label="شهر" {...field('city')} value={form.city ?? ''} disabled={!form.province} onChange={(e) => patch({ city: e.target.value })}><option value="">{form.province ? 'انتخاب شهر' : 'ابتدا استان را انتخاب کنید'}</option>{form.city && !availableCities.some((city) => city.name === form.city) ? <option value={form.city}>{form.city}</option> : null}{availableCities.map((city) => <option key={`${city.province}-${city.name}`} value={city.name}>{city.name}</option>)}</Select> : <Input label="شهر" {...field('city')} value={form.city ?? ''} onChange={(e) => patch({ city: withoutDigits(e.target.value) })} />}<Input label="کد پستی" {...field('postal_code')} dir="ltr" inputMode="numeric" maxLength={10} value={form.postal_code ?? ''} onChange={(e) => patch({ postal_code: e.target.value.replace(/\D/g, '').slice(0, 10) })} />

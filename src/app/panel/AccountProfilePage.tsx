  ...
  return <PanelPage index="ID.01" title="پرونده هویتی شرکت‌کننده" description="اطلاعات صاحب حساب را کامل کنید. سرپرست، مربی و اعضای تیم در پرونده همان تیم ثبت می‌شوند و حساب مستقل نمی‌سازند.">
    ...
    <form ref={formRef} className="space-y-5" noValidate onSubmit={(event) => void save(event)}>
      ...
      <PanelCard title="اطلاعات هویتی" description="فیلدهای ستاره‌دار الزامی هستند."><div className="grid gap-4 md:grid-cols-2">
        ...
        <div><Input label="شماره موبایل" {...field('phone')} dir="ltr" inputMode="numeric" maxLength={11} placeholder="09xxxxxxxxx" value={form.phone ?? ''} onChange={(e) => { patch({ phone: e.target.value.replace(/\D/g, '').slice(0, 11), phone_verified_at: null }); setOtpSent(false); setOtpChallengeId('') }} />{form.is_foreign ? <p className="mt-2 text-xs text-slate-500">برای شرکت‌کننده خارج از ایران، احراز هویت حساب از مسیر ایمیل انجام می‌شود.</p> : !form.phone_verified_at ? <div className="mt-2 flex gap-2">{otpSent ? <Input label="کد تأیید" dir="ltr" inputMode="numeric" value={otpCode} onChange={(e) => setOtpCode(e.target.value.replace(/\D/g, '').slice(0, 6))} maxLength={6} /> : null}<Button type="button" variant="secondary" onClick={() => void handleProfileOtp()}>{otpSent ? 'تأیید کد' : 'ارسال کد'}</Button></div> : <p className="mt-2 text-xs font-bold text-emerald-600">شماره موبایل تأیید شده است.</p>}</div>

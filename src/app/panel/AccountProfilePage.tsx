...
export function AccountProfilePage() {
  ...
  const cityOptions = useMemo(() => {
    const options = availableCities.map((city) => ({ value: city.name, label: city.name }))
    if (form?.city && !options.some((option) => option.value === form.city)) options.unshift({ value: form.city, label: form.city })
    return options
  ...
  return <PanelPage index="ID.01" title="پرونده هویتی شرکت‌کننده" description="اطلاعات صاحب حساب را کامل کنید.">
    ...
    <form ref={formRef} className="space-y-5" noValidate onSubmit={(event) => void save(event)}>
      ...
      <PanelCard title="اطلاعات هویتی" description="فیلدهای ستاره‌دار الزامی هستند."><div className="grid gap-4 md:grid-cols-2">
        ...
        {countryCode === 'IR' ? <Select label="استان" {...field('province')} value={form.province ?? ''} onChange={(e) => patch({ province: e.target.value, city: '' })}><option value="">انتخاب استان</option>{IRAN_PROVINCES.map((province)=><option key={province} value={province}>{province}</option>)}</Select> : <Input label="استان / منطقه" {...field('province')} value={form.province ?? ''} onChange={(e) => patch({ province: withoutDigits(e.target.value), city: '' })} />}{countryCode === 'IR' ? <CitySelect label="شهر" value={form.city ?? ''} options={cityOptions} placeholder={form.province ? 'انتخاب شهر' : 'ابتدا استان را انتخاب کنید'} required={required('city')} error={errors.city} disabled={!form.province} onChange={(city) => patch({ city })} /> : <Input label="شهر" {...field('city')} value={form.city ?? ''} onChange={(e) => patch({ city: withoutDigits(e.target.value) })} />}<Input label="کد پستی" {...field('postal_code')} dir="ltr" inputMode="numeric" maxLength={10} value={form.postal_code ?? ''} onChange={(e) => patch({ postal_code: e.target.value.replace(/\D/g, '').slice(0, 10) })} />

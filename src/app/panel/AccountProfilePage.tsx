  ...
  return <PanelPage index="ID.01" title="پرونده هویتی شرکت‌کننده" description="اطلاعات صاحب حساب را کامل کنید. سرپرست، مربی و اعضای تیم در پرونده همان تیم ثبت می‌شوند و حساب مستقل نمی‌سازند.">
    ...
    <form ref={formRef} className="space-y-5" noValidate onSubmit={(event) => void save(event)}>
      ...
      <PanelCard title="اطلاعات هویتی" description="فیلدهای ستاره‌دار الزامی هستند."><div className="grid gap-4 md:grid-cols-2">
        ...
        {form.country_code === 'IR' ? <Select label="استان" {...field('province')} value={form.province ?? ''} onChange={(e) => patch({ province: e.target.value })}><option value="">انتخاب استان</option>{IRAN_PROVINCES.map((province)=><option key={province} value={province}>{province}</option>)}</Select> : <Input label="استان / منطقه" {...field('province')} value={form.province ?? ''} onChange={(e) => patch({ province: withoutDigits(e.target.value) })} />}<Input label="شهر" {...field('city')} value={form.city ?? ''} onChange={(e) => patch({ city: withoutDigits(e.target.value) })} /><Input label="محل سکونت" {...field('residence')} value={form.residence ?? ''} onChange={(e) => patch({ residence: e.target.value })} /><Input label="کد پستی" {...field('postal_code')} dir="ltr" inputMode="numeric" maxLength={10} value={form.postal_code ?? ''} onChange={(e) => patch({ postal_code: e.target.value.replace(/\D/g, '').slice(0, 10) })} />

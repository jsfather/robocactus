...
export function BlogPostPage() {
  ...
  if (loading) return <div className="px-4 pb-20 pt-40 text-center text-slate-500">{t('app.loading')}</div>
  if (!post) return <div className="mx-auto max-w-3xl px-4 pb-20 pt-40 text-center"><p className="font-black text-red-600">{error ?? t('content.postNotFound')}</p><Link to="/blog" className="mt-5 inline-flex text-sky-700 hover:underline">{t('content.backToBlog')}</Link></div>

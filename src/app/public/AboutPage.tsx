...
export function AboutPage() {
  ...
  useEffect(() => {
    let mounted = true
    void fetchStaticPage('about').then((value) => { if (mounted) setPage(value) }).catch(() => undefined).finally(() => { if (mounted) setLoading(false) })
    return () => { mounted = false }
  }, [])
  const cmsBody = (en ? page?.body_en : page?.body) || ''
  const cover = page?.cover_image || page?.og_image
  const heroImage = page?.hero_image || cover
  const scopeImage = page?.scope_image
  const displayTitle = (en ? page?.title_en : page?.title)?.trim() || c.title
  const displayLead = (en ? page?.excerpt_en : page?.excerpt)?.trim() || c.lead
  usePageSeo({ title: displayTitle, description: displayLead, image: heroImage || undefined })
  if (loading) return <div className="mx-auto max-w-7xl px-4 pb-20 pt-40 text-center text-rc-muted">در حال بارگذاری محتوای درباره ما…</div>
  return <div className="pb-20"><section className="relative min-h-[72vh] overflow-hidden bg-[#052f46] text-white">{heroImage ? <img src={heroImage} alt="" className="absolute inset-x-0 bottom-0 h-[72%] w-full object-cover opacity-30 blur-[2px]" /> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_75%_20%,#0ea5a055,transparent_35%),linear-gradient(135deg,#052f46,#087eb8_55%,#087a58)]" />}<div className="absolute inset-0 bg-gradient-to-l from-[#042c42]/95 via-[#053c56]/85 to-[#042c42]/60" /><div className="relative mx-auto flex min-h-[72vh] max-w-7xl items-end px-4 pb-20 pt-28 sm:px-8 sm:pt-36 lg:pt-40"><motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl"><p className="text-xs font-black tracking-[.2em] text-cyan-200">{c.eyebrow}</p><h1 className="mt-4 text-4xl font-black leading-tight sm:text-6xl lg:text-7xl">{displayTitle}</h1><p className="mt-6 max-w-3xl text-base leading-8 text-white/78 sm:text-lg">{displayLead}</p></motion.div></div></section><main className="mx-auto max-w-7xl space-y-20 px-4 py-20 sm:px-8"><section className="grid gap-8 lg:grid-cols-[.8fr_1.2fr] lg:items-start"><div><p className="text-xs font-black tracking-[.2em] text-sky-600">ABOUT TABARESTAN CUP</p><h2 className="mt-3 text-3xl font-black text-slate-900 sm:text-5xl">{c.national}</h2>{scopeImage ? <img src={scopeImage} alt="" loading="lazy" className="mt-7 aspect-[4/3] w-full rounded-[2rem] object-cover shadow-[0_20px_50px_rgb(7_59_85/0.15)]" /> : null}</div><div className="text-base leading-9 text-slate-600">{cmsBody ? <div className="space-y-4" dangerouslySetInnerHTML={{ __html: sanitizeHtml(cmsBody) }} /> : <p>{c.intro}</p>}</div></section><section><p className="text-xs font-black tracking-[.2em] text-sky-600">{c.values}</p><div className="mt-6 grid gap-5 md:grid-cols-2">{[c.mission, c.vision].map(([title, body], index) => <article key={title} className={`rounded-[2rem] p-7 text-white shadow-xl ${index ? 'bg-gradient-to-br from-[#087a58] to-[#0a9d77]' : 'bg-gradient-to-br from-[#063d59] to-[#087eb8]'}`}><span className="grid size-12 place-items-center rounded-2xl bg-white/15 text-lg font-black"><svg viewBox="0 0 24 24" className="size-6" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><circle cx="12" cy="12" r="8" /><path d="m12 7 2 5-2 2-2-2 2-5Z" /></svg></span><h3 className="mt-6 text-2xl font-black">{title}</h3><p className="mt-3 leading-8 text-white/75">{body}</p></article>)}</div></section><section><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black tracking-[.2em] text-emerald-600">TECHNOLOGY & COMPETITION</p><h2 className="mt-3 text-3xl font-black text-slate-900">{c.activity}</h2></div></div><div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{c.fields.map(([title, body]) => <article key={title} className="group rounded-[1.75rem] border border-slate-100 bg-white p-6 shadow-[0_14px_40px_rgb(7_59_85/0.07)] transition hover:-translate-y-1 hover:border-sky-200"><span className="grid size-11 place-items-center rounded-2xl bg-gradient-to-br from-sky-50 to-emerald-50 text-sky-700"><svg viewBox="0 0 24 24" className="size-5" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M4 8h16M6 4h12v16H6z" /><path d="M9 12h6M9 16h4" /></svg></span><h3 className="mt-5 text-lg font-black text-slate-900">{title}</h3><p className="mt-3 text-sm leading-7 text-slate-500">{body}</p></article>)}</div></section><section className="overflow-hidden rounded-[2rem] bg-gradient-to-l from-[#063d59] via-[#087eb8] to-[#087a58] p-7 text-white sm:p-10"><div className="grid gap-6 md:grid-cols-[1fr_auto] md:items-center"><div><h2 className="text-2xl font-black sm:text-4xl">{c.scope}</h2><p className="mt-4 max-w-3xl leading-8 text-white/75">{c.scopeBody}</p></div><div className="flex flex-col gap-3"><Link to="/leagues" className="rounded-2xl bg-white px-6 py-3 text-center text-sm font-black text-sky-800">{c.cta}</Link><Link to="/registration-guide" className="rounded-2xl border border-white/25 bg-white/10 px-6 py-3 text-center text-sm font-black">{c.guide}</Link></div></div></section></main></div>
}

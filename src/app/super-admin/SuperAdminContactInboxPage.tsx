        ...
        <div className="grid min-h-[650px] lg:grid-cols-[minmax(300px,0.82fr)_minmax(0,1.5fr)]">
          <aside className="border-b border-rc-line bg-slate-50/70 lg:border-b-0 lg:border-e">
            ...
            <div className="max-h-[530px] overflow-y-auto p-2">
              ...
              {filtered.map((message) => <button key={message.id} type="button" onClick={() => setSelectedId(message.id)} className={`mb-2 w-full rounded-2xl border p-4 text-start transition ${selectedId === message.id ? 'border-rc-blue/35 bg-white shadow-[0_10px_30px_rgb(18_76_98/0.11)]' : 'border-transparent hover:border-rc-line hover:bg-white'}`}><span className="flex items-start justify-between gap-3"><strong className="line-clamp-1 text-sm text-rc-text">{message.full_name}</strong><span className={`shrink-0 rounded-full border px-2 py-1 text-[10px] font-bold ${statusMeta[message.status].style}`}>{fa ? statusMeta[message.status].fa : statusMeta[message.status].en}</span></span><span className="mt-2 block line-clamp-1 text-sm font-medium text-rc-text">{message.subject}</span><span className="mt-1 block line-clamp-2 text-xs leading-5 text-rc-muted">{message.body}</span><time className="mt-2 block text-[11px] text-rc-muted">{formatAppDateTime(message.created_at, i18n.language)}</time></button>)}
            </div>
          </aside>
          <main className="p-5 md:p-7">
            {!selected ? <div className="grid h-full place-items-center text-center text-sm text-rc-muted">{fa ? 'یک پیام را از فهرست انتخاب کنید.' : 'Select a message from the list.'}</div> : (
              <div className="mx-auto max-w-3xl">
                <div className="flex flex-wrap items-start justify-between gap-4 border-b border-rc-line pb-5"><div><span className={`inline-flex rounded-full border px-3 py-1 text-xs font-bold ${statusMeta[selected.status].style}`}>{fa ? statusMeta[selected.status].fa : statusMeta[selected.status].en}</span><h2 className="mt-3 text-xl font-black text-rc-text">{selected.subject}</h2><time className="mt-1 block text-xs text-rc-muted">{formatAppDateTime(selected.created_at, i18n.language)}</time></div><div className="flex gap-2"><a href={`mailto:${selected.email}`} className="rounded-xl border border-rc-line px-3 py-2 text-xs font-bold text-rc-blue hover:bg-rc-blue/5">{fa ? 'ارسال ایمیل' : 'Email'}</a>{selected.phone ? <a href={`tel:${selected.phone}`} className="rounded-xl border border-rc-line px-3 py-2 text-xs font-bold text-rc-blue hover:bg-rc-blue/5">{fa ? 'تماس' : 'Call'}</a> : null}</div></div>

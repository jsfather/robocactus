...
export function SuperAdminKavenegarPage() {
  ...
  return <PanelPage index="SYS.SMS" title="مرکز عملیات کاوه‌نگار" description="ارسال، گزارش تحویل، صندوق پیام، الگوها، رسانه و تنظیمات حساب از یک پنل امن" actions={<Button type="button" variant="secondary" onClick={() => void reload()}>به‌روزرسانی</Button>}>
    ...
    {tab === 'send' ? <div className="grid gap-5 xl:grid-cols-2">
      <PanelCard title="ارسال پیامک حرفه‌ای" description="ارسال فوری یا زمان‌بندی‌شده با خط، تگ، جریان ارسال و رسانه">
        <form className="grid gap-4 md:grid-cols-2" onSubmit={sendSimple}>
          ...
          <Input label="زمان ارسال" type="datetime-local" value={send.date} onChange={(event) => setSend({ ...send, date: event.target.value })} />
      ...
      <PanelCard title="ارسال گروهی SendArray" description="هر سطر گیرنده، فرستنده و متن با سطر متناظر خود ارسال می‌شود؛ سقف هر درخواست ۲۰۰ رکورد">
        <form className="grid gap-4" onSubmit={sendBulk}><div className="grid gap-4 md:grid-cols-3"><Textarea label="گیرنده‌ها؛ هر خط یک شماره" required value={bulk.receptors} onChange={(event) => setBulk({ ...bulk, receptors: event.target.value })} dir="ltr" /><Textarea label="فرستنده‌ها؛ هر خط یک شماره" required value={bulk.senders} onChange={(event) => setBulk({ ...bulk, senders: event.target.value })} dir="ltr" /><Textarea label="متن‌ها؛ هر خط یک پیام" required value={bulk.messages} onChange={(event) => setBulk({ ...bulk, messages: event.target.value })} /></div><div className="grid gap-4 md:grid-cols-3"><Input label="زمان ارسال" type="datetime-local" value={bulk.date} onChange={(event) => setBulk({ ...bulk, date: event.target.value })} /><Input label="تگ" value={bulk.tag} onChange={(event) => setBulk({ ...bulk, tag: event.target.value })} dir="ltr" /><Input label="Policy" value={bulk.policy} onChange={(event) => setBulk({ ...bulk, policy: event.target.value })} dir="ltr" /></div><Button type="submit" disabled={busy}>ارسال گروهی</Button></form>
      </PanelCard>
    ...
    {tab === 'templates' ? <Workbench title="مدیریت کامل الگوهای اعتبارسنجی" presets={templatePresets} onComplete={completeWorkbench} /> : null}

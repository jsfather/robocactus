...
export function SuperAdminCompetitionSettingsPage() {
  ...
  return <PanelPage index="REG.03" title="تنظیمات مسابقات" description="رزومه مشترک داوران و کمیته فنی، اسپانسرها و وضعیت داوری هر لیگ را یک‌جا مدیریت کنید.">
    ...
    {tab === 'people' ? <div className="grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(20rem,0.9fr)]">
      ...
      </PanelCard> : <PanelCard title="وضعیت داوری لیگ‌ها" description="داوری هر لیگ مستقل از رزومه افراد فعال یا غیرفعال می‌شود."><div className="grid gap-3">{leagues.map((league) => <label key={league.id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 p-4"><span><b>{league.name}</b><small className="mt-1 block text-xs text-slate-500">{league.judging_enabled === false ? 'داوری غیرفعال؛ ثبت دستی برندگان' : 'داوری فعال'}</small></span><input type="checkbox" checked={league.judging_enabled !== false} disabled={busy} onChange={(e) => void toggleJudging(league, e.target.checked)} /></label>)}</div></PanelCard>}

...
export function SuperAdminUsersPage() {
  ...
  const signupStepLabels: Record<string, string> = { type: 'نوع حساب', channel: 'روش ثبت‌نام', identity: 'اطلاعات هویتی', verify: 'تأیید ارتباط', docs: 'مدارک', review: 'تأیید نهایی' }
  ...
  const filteredParticipants = useMemo(() => participants.filter((profile) => {
    ...
    return (!search.trim() || haystack.includes(search.trim().toLowerCase()))
      ...
      && (completionFilter === 'all'
        || (completionFilter === 'registration_incomplete' && Boolean(profile.requires_account_approval && !profile.signup_completed_at))
        || (completionFilter === 'complete' && Boolean(profile.identity_completed_at))
        || (completionFilter === 'incomplete' && !profile.identity_completed_at))

...
function WithdrawalSettingsForm({leagueId}:{leagueId:string}){
  const [value,setValue]=useState<AttendanceSettings|null>(null),[busy,setBusy]=useState(false),[message,setMessage]=useState('')
  useEffect(()=>{void backend.from('league_attendance_settings').select('*').eq('league_id',leagueId).maybeSingle().then(({data,error})=>{if(error)setMessage(error.message);else setValue(data as AttendanceSettings)})},[leagueId])
  if(!value)return <PanelCard title="شرایط انصراف"><p className="text-sm text-slate-500">{message||'در حال بارگذاری…'}</p></PanelCard>
  const save=async()=>{setBusy(true);setMessage('');const {error}=await backend.from('league_attendance_settings').update({withdrawal_enabled:value.withdrawal_enabled,withdrawal_deadline:value.withdrawal_deadline||null,withdrawal_terms_fa:value.withdrawal_terms_fa,withdrawal_terms_en:value.withdrawal_terms_en}).eq('league_id',leagueId);setBusy(false);setMessage(error?error.message:'تنظیمات انصراف ذخیره شد.')}
  return <PanelCard title="شرایط و مهلت انصراف" description="این متن هنگام پذیرش قوانین لیگ و پس از صدور مجوز به شرکت‌کننده نمایش داده می‌شود."><div className="grid gap-4 md:grid-cols-2"><label className="flex items-center gap-3 rounded-xl border border-slate-200 p-4 text-sm font-bold"><input type="checkbox" checked={value.withdrawal_enabled} onChange={e=>setValue({...value,withdrawal_enabled:e.target.checked})}/>امکان درخواست انصراف فعال باشد</label><DateTimeField label="مهلت درخواست انصراف" value={value.withdrawal_deadline??''} onChange={next=>setValue({...value,withdrawal_deadline:next})}/><Textarea label="شرایط انصراف (فارسی)" value={value.withdrawal_terms_fa??''} onChange={e=>setValue({...value,withdrawal_terms_fa:e.target.value})}/><Textarea label="Withdrawal terms" dir="ltr" value={value.withdrawal_terms_en??''} onChange={e=>setValue({...value,withdrawal_terms_en:e.target.value})}/></div><div className="mt-4 flex items-center gap-3"><Button type="button" disabled={busy} onClick={()=>void save()}>{busy?'در حال ذخیره…':'ذخیره شرایط انصراف'}</Button>{message?<span className="text-sm text-slate-600">{message}</span>:null}</div></PanelCard>
}

...
export function SuperAdminParticipantsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (seasonYear) teamQuery = teamQuery.eq('season_year', Number(seasonYear))
  ...
  useEffect(() => { void load() }, [leagueId, seasonYear])
  const companyById = useMemo(() => new Map(companies.map((company) => [company.id, company])), [companies])
  const ownerById = useMemo(() => new Map(owners.map((owner) => [owner.id, owner])), [owners])
  const visibleTeams = useMemo(() => { const needle = query.trim().toLocaleLowerCase('fa'); if (!needle) return teams; return teams.filter((team) => { const company = companyById.get(team.company_id); const owner = ownerById.get(team.captain_id); const captain = members.find((member) => member.team_id === team.id && member.role === 'captain'); return [team.name, team.name_en, company?.name, owner?.full_name, owner?.phone, captain?.full_name, captain?.phone].some((value) => value?.toLocaleLowerCase('fa').includes(needle)) }) }, [companyById, members, ownerById, query, teams])
  const selectedLeague = leagues.find((league) => league.id === leagueId)
  const seasonYears = useMemo(() => [...new Set(leagues.map((league) => Number(league.current_season_year)).filter(Boolean))].sort((a, b) => b - a), [leagues])

  return <PanelPage index="REG.06" title="اطلاعات تیم‌ها" description="تیم، مجموعه ثبت‌نام‌کننده، مالک حساب، سرپرست و اعضای هر ثبت‌نام را یکجا ببینید.">
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard index="01" label="مجموعه‌ها" value={new Set(teams.map((team) => team.company_id)).size} hint="اشخاص حقیقی و حقوقی" /><StatCard index="02" label="تیم‌ها" value={teams.length} hint="تیم‌های وابسته به مجموعه‌ها" accent="green" /><StatCard index="03" label="افراد تیم" value={members.length} hint="سرپرست، مربی و اعضا" accent="orange" /><StatCard index="04" label="دوره" value={seasonYear ? formatSeasonYear(Number(seasonYear), 'fa') : selectedLeague ? formatSeasonYear(selectedLeague.current_season_year, 'fa') : 'همه'} hint="دوره انتخاب‌شده" /></div>
    <PanelCard title="جست‌وجو و فیلتر" description="با نام مجموعه، مالک حساب، موبایل، سرپرست یا نام تیم جست‌وجو کنید."><div className="grid gap-3 md:grid-cols-3"><Select label="لیگ" value={leagueId} onChange={(event) => setLeagueId(event.target.value)}><option value="">همه لیگ‌ها</option>{leagues.map((league) => <option key={league.id} value={league.id}>{league.name} · {formatSeasonYear(league.current_season_year, 'fa')}</option>)}</Select><Select label="سال دوره" value={seasonYear} onChange={(event) => setSeasonYear(event.target.value)}><option value="">همه سال‌ها</option>{seasonYears.map((year) => <option key={year} value={year}>{formatSeasonYear(year, 'fa')}</option>)}</Select><Input label="نام مجموعه، مالک، سرپرست یا تیم" value={query} onChange={(event) => setQuery(event.target.value)} /></div></PanelCard>
    ...
    <PanelCard title="مجموعه‌ها و تیم‌های ثبت‌شده">{loading ? <p className="text-rc-muted">در حال بارگذاری…</p> : <div className="space-y-3">{visibleTeams.map((team) => { const teamMembers = members.filter((member) => member.team_id === team.id); const company = companyById.get(team.company_id); const owner = ownerById.get(team.captain_id); return <article key={team.id} className={`overflow-hidden rounded-2xl border bg-white ${expanded === team.id ? 'border-sky-300 ring-4 ring-sky-50' : 'border-slate-200'}`}>
      <button type="button" className="grid w-full gap-4 p-5 text-start md:grid-cols-[minmax(0,1fr)_minmax(13rem,.65fr)_minmax(13rem,.65fr)_auto] md:items-center" onClick={() => setExpanded(expanded === team.id ? null : team.id)} aria-expanded={expanded === team.id}><span className="min-w-0"><small className="font-bold text-sky-700">مجموعه</small><strong className="mt-1 block truncate text-base text-slate-900">{company?.name ?? 'مجموعه بدون نام'}</strong><span className="mt-1 block text-xs text-slate-500">تیم: {team.name}{team.name_en ? ` / ${team.name_en}` : ''}</span></span><span className="min-w-0 rounded-xl bg-slate-50 px-4 py-3"><small className="block text-slate-500">مالک و مدیر حساب</small><strong className="mt-1 block truncate text-sm text-slate-800">{owner?.full_name ?? 'پروفایل یافت نشد'}</strong><span className="mt-1 block text-xs text-slate-500" dir="ltr">{owner?.phone ?? owner?.email ?? '—'}</span></span><span className="min-w-0 rounded-xl bg-sky-50 px-4 py-3"><small className="block text-sky-700">سرپرست تیم</small><strong className="mt-1 block truncate text-sm text-slate-800">{teamMembers.find((member) => member.role === 'captain')?.full_name || 'ثبت نشده'}</strong><span className="mt-1 block text-xs text-slate-500" dir="ltr">{teamMembers.find((member) => member.role === 'captain')?.phone || '—'}</span></span><StatusBadge status={team.status} label={statusLabel[team.status] ?? team.status} /></button>

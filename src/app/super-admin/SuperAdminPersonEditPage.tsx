    ...
    <form className="space-y-6" onSubmit={(event) => void save(event)}>
      ...
      <PanelCard title="مشخصات اصلی و تصویر">
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <div className="space-y-2"><label className="flex min-h-11 items-center gap-3 rounded-xl border border-slate-200 px-3 text-sm font-bold"><input type="checkbox" checked={form.is_founder === true} onChange={(event) => patch({ is_founder: event.target.checked })} /> بنیان‌گذار جام تبرستان</label><ImageUploadField label="نشان بنیان‌گذار جام تبرستان" value={form.founder_badge_url ?? null} onChange={(url) => patch({ founder_badge_url: url })} /></div>

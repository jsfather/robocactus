...
export function StaffPage({ section = 'tickets' }: { section?: 'tickets' | 'triage' }) {
  ...
  return (
    ...
    >
      ...
      {selectedAccount ? <div className="fixed inset-0 z-[180] flex items-start justify-center overflow-y-auto bg-slate-950/70 p-3 py-4 backdrop-blur-sm sm:p-6 sm:py-8" role="dialog" aria-modal="true" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedAccount(null) }}>
        <div className="my-auto flex max-h-[calc(100dvh-2rem)] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-slate-50 shadow-2xl sm:max-h-[calc(100dvh-4rem)] sm:rounded-3xl">

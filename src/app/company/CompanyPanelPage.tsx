        ...
        <>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <PanelCard title="لیگ‌های پیش رو" description="لیگ‌های فعال و آماده ثبت‌نام برای مجموعه شما">
              {leagues.length ? <div className="space-y-2">{leagues.map((league) => <div key={league.id} className="flex items-center justify-between gap-3 border-b border-rc-line/60 py-3 last:border-0"><div><p className="font-bold text-slate-800">{league.name}</p><p className="mt-1 text-xs text-rc-muted">{league.registration_close_at ? `مهلت ثبت‌نام: ${new Date(league.registration_close_at).toLocaleDateString('fa-IR')}` : 'ثبت‌نام فعال'}</p></div><Link to="/company/competitions" className="shrink-0 text-sm font-bold text-rc-blue">مشاهده و ثبت‌نام</Link></div>)}</div> : <p className="text-sm text-rc-muted">در حال حاضر لیگ فعالی برای ثبت‌نام وجود ندارد.</p>}
            </PanelCard>

...
export function PublicHeader() {
  ...
  return <div className="pointer-events-none fixed inset-x-0 top-0 z-50 px-2 pt-2 sm:px-5 sm:pt-4">
    ...
    <header ref={headerRef} className="pointer-events-auto relative mx-auto max-w-7xl border border-slate-200 bg-white/96 shadow-[0_8px_28px_rgb(15_23_42/0.09)] backdrop-blur-xl">
      ...
      <div onMouseEnter={openMega} onMouseLeave={closeMega} className={`absolute inset-x-0 top-full pt-2 transition duration-200 motion-reduce:transition-none ${megaOpen ? 'visible translate-y-0 opacity-100' : 'invisible -translate-y-1 opacity-0'}`}>
        <section className="mx-auto grid max-w-4xl border border-slate-200 bg-white shadow-[0_20px_50px_rgb(15_23_42/0.14)] lg:grid-cols-[15rem_1fr]" aria-label={t('nav.leagues')}>
      ...
      <div id="public-mobile-menu" className={`absolute inset-x-0 top-full mt-2 max-h-[calc(100dvh-6rem)] overflow-y-auto border border-slate-200 bg-white shadow-[0_20px_50px_rgb(15_23_42/0.18)] transition duration-200 motion-reduce:transition-none xl:hidden ${mobileOpen ? 'visible translate-y-0 opacity-100' : 'invisible -translate-y-2 opacity-0'}`}>

...
const desktopNavClass = ({ isActive }: { isActive: boolean }) => `relative inline-flex min-h-10 items-center rounded-xl px-3 text-sm font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rc-blue ${isActive ? 'bg-gradient-to-l from-sky-50 to-emerald-50 text-rc-blue shadow-sm ring-1 ring-sky-100' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950'}`
...
export function PublicHeader() {
  ...
  useEffect(() => {
    ...
    return () => { document.body.style.overflow = previousOverflow; document.removeEventListener('pointerdown', close); document.removeEventListener('keydown', escape) }
  }, [mobileOpen])
  ...
  return <div className="pointer-events-none fixed inset-x-0 top-0 z-[100] bg-transparent">
    ...
    <header ref={headerRef} className="pointer-events-auto relative mx-auto max-w-7xl bg-transparent px-2 sm:px-4 lg:px-5">
      <div className="hidden h-11 items-center justify-between rounded-t-[1.4rem] bg-gradient-to-l from-[#087eb8] via-[#078a9b] to-[#0b9b65] px-5 text-white lg:flex">
        ...
        <div className="flex h-full items-center gap-3 text-xs text-white/90 [&_button]:!border-white/25 [&_button]:!bg-transparent [&_button]:!text-white [&_button:hover]:!bg-white [&_button:hover]:!text-rc-blue [&_a]:!text-white [&_span]:!border-white/25">
      ...
      <div className="relative flex h-[4.25rem] items-center overflow-hidden rounded-b-2xl border border-t-0 border-slate-100 bg-white px-1 shadow-[0_10px_35px_rgb(15_65_80/0.12)] lg:rounded-b-[1.4rem]"><span className="absolute inset-x-0 top-0 h-1 bg-gradient-to-l from-rc-blue to-rc-accent lg:hidden" aria-hidden />
        ...
        <nav className="ms-auto hidden items-center gap-1 xl:flex" aria-label={locale === 'en' ? 'Main navigation' : 'منوی اصلی'}>
        ...
        <div className="ms-auto flex items-center gap-1.5 pe-1 xl:ms-2">
          ...
          {user ? <Link to="/dashboard" className="hidden min-h-10 items-center rounded-xl bg-gradient-to-l from-rc-blue to-rc-accent px-5 text-sm font-black text-white shadow-sm transition hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rc-blue md:inline-flex">{t('nav.dashboard')}</Link> : <div className="hidden items-center gap-1.5 md:flex"><Link to="/login" className="inline-flex min-h-10 items-center rounded-xl border border-sky-100 bg-sky-50 px-4 text-sm font-black text-rc-blue transition hover:bg-sky-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rc-blue">{t('nav.login')}</Link><Link to="/signup" className="inline-flex min-h-10 items-center rounded-xl bg-gradient-to-l from-rc-blue to-rc-accent px-5 text-sm font-black text-white shadow-sm transition hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rc-blue">{t('nav.signup')}</Link></div>}
          <button type="button" className={`${controlClass} xl:hidden`} onClick={() => { setMegaOpen(false); setMobileLeaguesOpen(false); setMobileOpen((value) => !value) }} aria-label={locale === 'en' ? 'Navigation menu' : 'منوی ناوبری'} aria-expanded={mobileOpen} aria-controls="public-mobile-menu"><LineIcon>{mobileOpen ? <path d="M5 5l14 14M19 5 5 19" /> : <><path d="M4 7h16M4 12h16M4 17h16" /></>}</LineIcon></button>
        </div>
      </div>

      <div onMouseEnter={openMega} onMouseLeave={closeMega} className={`absolute inset-x-5 top-full hidden pt-2 transition duration-200 motion-reduce:transition-none xl:block ${megaOpen ? 'visible translate-y-0 opacity-100' : 'invisible -translate-y-2 opacity-0'}`}>
        <section className="overflow-hidden rounded-[1.4rem] border border-sky-100 bg-white p-3 shadow-[0_24px_55px_rgb(15_65_80/0.16)]" aria-label={t('nav.leagues')}>
          <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl bg-gradient-to-l from-sky-50 to-emerald-50 px-5 py-4"><div><p className="text-[10px] font-black tracking-[.14em] text-emerald-700">{locale === 'en' ? 'COMPETITIONS' : 'رقابت‌ها'}</p><h2 className="mt-1 text-lg font-black text-slate-900">{locale === 'en' ? 'Active robotics leagues' : 'لیگ‌های فعال رباتیک'}</h2><p className="mt-1 text-xs text-slate-500">{locale === 'en' ? 'Rules, schedules and registration status' : 'قوانین، زمان‌بندی و وضعیت ثبت‌نام هر لیگ'}</p></div><Link to="/leagues" onClick={() => setMegaOpen(false)} className="inline-flex min-h-10 items-center rounded-xl bg-gradient-to-l from-rc-blue to-rc-accent px-4 text-sm font-black text-white shadow-sm">{locale === 'en' ? 'View all leagues' : 'مشاهده همه لیگ‌ها'}<span className="ms-2" aria-hidden="true">←</span></Link></div>
          <div className="mt-3 grid max-h-[25rem] gap-2 overflow-y-auto sm:grid-cols-2 lg:grid-cols-3">{activeLeagues.slice(0, 9).map((league) => <Link key={league.id} to={`/leagues/${league.slug}`} onClick={() => setMegaOpen(false)} className="group flex min-h-20 items-center gap-3 rounded-xl border border-slate-100 p-3 transition hover:border-sky-100 hover:bg-sky-50/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rc-blue"><span className="grid size-12 shrink-0 place-items-center overflow-hidden rounded-xl bg-gradient-to-br from-sky-50 to-emerald-50 ring-1 ring-sky-100">{league.cover_image_url ? <img src={league.cover_image_url} alt="" className="size-full object-cover" /> : <LineIcon className="size-5 text-rc-blue"><path d="M8 4h8v4a4 4 0 0 1-8 0V4Z" /><path d="M8 6H4v1a4 4 0 0 0 4 4M16 6h4v1a4 4 0 0 1-4 4M12 12v4M8 20h8" /></LineIcon>}</span><span className="min-w-0"><strong className="block truncate text-sm text-slate-900 group-hover:text-rc-blue">{locale === 'en' ? league.name_en || league.name : league.name}</strong><small className={`mt-1 inline-flex rounded-full px-2 py-1 text-[10px] font-bold ${league.registration_cycle_status === 'open' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-50 text-slate-500'}`}>{league.registration_cycle_status === 'open' ? (locale === 'en' ? 'Registration open' : 'ثبت‌نام فعال') : (locale === 'en' ? 'League details' : 'جزئیات لیگ')}</small></span></Link>)}{!activeLeagues.length ? <p className="p-8 text-center text-sm text-slate-500 sm:col-span-2 lg:col-span-3">{locale === 'en' ? 'No active leagues.' : 'در حال حاضر لیگ فعالی وجود ندارد.'}</p> : null}</div>
        </section>
      </div>
      ...
      <div id="public-mobile-menu" className={`absolute inset-x-0 top-full max-h-[calc(100dvh-4.25rem)] overflow-y-auto border-b border-slate-200 bg-white shadow-[0_16px_36px_rgb(15_23_42/0.12)] transition duration-200 motion-reduce:transition-none xl:hidden ${mobileOpen ? 'visible translate-y-0 opacity-100' : 'invisible -translate-y-2 opacity-0'}`}>
        ...
        {activeLeagues.length ? <div className="border-b border-slate-200"><button type="button" className="flex min-h-12 w-full items-center justify-between px-5 text-sm font-black text-slate-700" onClick={() => setMobileLeaguesOpen((value) => !value)} aria-expanded={mobileLeaguesOpen} aria-controls="mobile-active-leagues"><span>{locale === 'en' ? 'Active leagues' : 'لیگ‌های فعال'}</span><LineIcon className={`size-4 transition-transform ${mobileLeaguesOpen ? 'rotate-180' : ''}`}><path d="m7 10 5 5 5-5" /></LineIcon></button>{mobileLeaguesOpen ? <div id="mobile-active-leagues" className="grid gap-2 bg-slate-50 p-4 sm:grid-cols-2">{activeLeagues.slice(0, 4).map((league) => <Link key={league.id} to={`/leagues/${league.slug}`} onClick={() => { setMobileLeaguesOpen(false); setMobileOpen(false) }} className="min-h-11 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700">{locale === 'en' ? league.name_en || league.name : league.name}</Link>)}</div> : null}</div> : null}

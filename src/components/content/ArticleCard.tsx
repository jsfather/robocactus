...
export function ArticleCard({ to, title, excerpt, image, imageAlt, publishedAt, category, kind = 'article', featured }: Props) {
  ...
  return <Link to={to} className={`group flex h-full w-full min-w-0 max-w-full overflow-hidden rounded-[1.75rem] border border-sky-100 bg-white shadow-[0_16px_48px_rgb(18_76_98/0.08)] transition duration-300 hover:-translate-y-1 hover:border-sky-200 hover:shadow-[0_24px_60px_rgb(18_76_98/0.14)] ${featured ? 'flex-col sm:grid sm:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)]' : 'flex-col'}`}>
    ...
    <div className="flex min-w-0 max-w-full flex-1 flex-col overflow-hidden p-5 sm:p-6"><div className="flex min-w-0 flex-wrap items-center gap-3 text-[11px] font-bold text-slate-500">{publishedAt ? <time>{formatAppDate(publishedAt, i18n.language)}</time> : null}<span>{minutes.toLocaleString(isEn ? 'en-US' : 'fa-IR')} {isEn ? 'min read' : 'دقیقه مطالعه'}</span></div><h2 className={`${featured ? 'mt-4 text-2xl' : 'mt-3 text-lg'} line-clamp-2 max-w-full break-words [overflow-wrap:anywhere] font-black leading-8 text-slate-900 transition group-hover:text-sky-700`}>{title}</h2>{excerpt ? <p className="mt-3 line-clamp-3 max-w-full break-words [overflow-wrap:anywhere] text-sm leading-7 text-slate-600">{excerpt}</p> : null}<span className="mt-auto inline-flex items-center gap-2 pt-5 text-sm font-black text-sky-700">{isEn ? 'Read more' : 'مطالعه مطلب'} <span className="transition group-hover:-translate-x-1" aria-hidden="true">←</span></span></div>
  </Link>
}
